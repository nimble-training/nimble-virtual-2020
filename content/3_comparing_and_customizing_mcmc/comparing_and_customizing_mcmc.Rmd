...
Code: If $\theta_1$ is `target <- 'theta[1]'`, the nodes needing calculation are obtained by `model$getDependencies(target)`.
...
- Remember that the target distribution may change on the next iteration (goldenrod distribution) because $\theta_F$ may have been updated.
...
- Mixing might typically be better than adaptive random-walk Metropolis-Hastings, but at high computational cost.
...
- It can also run other MCMC "engines" via a plug-in system.  Currently there is a plug-in for JAGS and a draft plug-in for Stan.  Plug-ins for JAGS and OpenBUGS/WinBUGS (not yet implemented) can use the same model as `nimble` if it is compatible.
