...
If `farm_ids` (and `sex`) are not known to NIMBLE to be constants, then all the observations could potentially depend on any element of `farm_effects`, `sex_int`, `length_coef`.
...
```{r}
model$getDependencies('intercept')
...
```{r}
model$y
simulate_downstream(model, 'sigma2')
model$y
```
