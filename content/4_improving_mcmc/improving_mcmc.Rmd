...
- Random effects with a mean of zero (non-centered) versus centered around a mean.
    - E.g., `farm_effect ~ N(0, sd)` vs. `farm.effect ~ N(mean, sd)`.
...
Four sets of model/MCMC configuration:
 ...
 - Original setting from last unit -- centered length covariate
 - Centering the random effects (still centered length covariate): "ctrRE"
 - Uncentered length covariate: "uncLen"
 - Uncentered length covariate and block intercept/slope pairs: "block"
...
DEcodeCtrRE <- nimbleCode({
  sex_effect[2] ~ dnorm(0, sd = 1000)     # offset for sex 2
  sex_effect[1] <- 0                      # replaced by 'mu'
  for(i in 1:2) {
...
ts.plot(DEsamplesCtrRE[ , 'sex_effect[2]'], main = 'sex 2 offset')
...
mcmcResults_jags_uncenLen <- compareMCMCs(
...
Results if you run the code yourself will be [here for uncentered length covariate](deer_ecervie_uncLen_results.html) and [here for the centered random effects (and centered length covariate)](deer_ecervie_ctrRE_results.html).
...
modelFlexMarg <- nimbleModel(DEcodeFlexMarg, data = DEdata, 
                     constants = DEconstants, 
                     inits = c(DEinits_vals, pi = runif(1), 
