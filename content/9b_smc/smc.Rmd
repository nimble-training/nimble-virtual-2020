...
Given the integration of the volatility process is not possible analytically, there are two potential approaches in NIMBLE:
  ...
  - IF2 SMC-based approach (not shown here)
...
```{r}
stochVolMCEM <- buildMCEM(stochVolModel, burnIn = 200, latentNodes = 'x')
MLEs <- stochVolMCEM$run(initM = 1000)
MLEs
```

When we were exploring this model, we realized that there is not a lot of information in the data to estimate the parameters, so there are some shortcomings to using this example with MCEM. But it does illustrate the idea and usage.
