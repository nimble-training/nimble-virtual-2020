...
MCMC <- buildMCMC(conf)
cMCMC <- compileNimble(MCMC, project = cModel)
samples <- runMCMC(cMCMC, niter = 30000, nburnin = 5000, thin = 25)
```
...
sample_xstar <- nimbleFunction({
  ...
  run = function(x, mu, sigma, rho, dist11, dist22, dist21) {
    # returnType(???)
    # insert code to take a sample given equation above
  }
})

get_samples <- nimbleFunction(
  # need types for inputs
  run = function(samples, dist11, dist22, dist21) {
    # returnType(???)
    m <- dim(samples)[1]
    # nstar <- ???
